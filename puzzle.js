...
function move(id){
	...
	if(target_d !=0){
		...
		//最后设置被点击的小div的位置，把它移动到目标大div的位置
	}
	//如果target_d不为0，可以移动，且其值为要移动去的编号
...
function timer(){
	...
	var sec=time%60;//秒
	document.getElementById("timer").innerHTML=min+"分"+sec+"秒";//更新时间
}
...
//随机打乱方块函数，思路是从第九快开始，随机生成一个数，然后让他两对调一下
function random_d(){
	for(var i=9;i>1;--i){
...
/*
function choose(version_id){
	if(version_id==1){
		for(var p=1;p<=8;p++){
			document.getElementById("d"+p).innerHTML=p;
		}
		
	}
	
	/*
	莫名出问题 究竟哪里错了 我好像dom方法弄错了吧
	if(version_id==2){
		for(var b=1;b<=8;b++){
			document.getElementById("d"+p).innerHTML="";
			document.getElementById("d"+b).innerHTML="<img src=\""+b".jpg\">";
